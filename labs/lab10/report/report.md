...
Для начала создадим каталог lab10, в котором с помощью vi создадим скриптовый файл hello.sh (рис. [-@fig:001]).

![Создание каталога и файла в нём с помощью vi](image/1.png){#fig:001}

Файл сразу же откроется в vi. Перейдём с помощью клавиши i в режим вставки и вставим следующий текст. Затем, нажмём ":" и напишем wq для сохранения файла и выхода (рис. [-@fig:002]).
...
С помощью стрелочек перейдём на конец слова HELLO и перейдём в режим вставки. Допишем букву "O" (рис. [-@fig:004]).

![Замена слов](image/4.png){#fig:004}

Теперь перейдём на строку, где заменим LOCAL на local (рис. [-@fig:005]).

![Замена слов](image/5.png){#fig:005}

Добавим строчку и сотрем ее, с помощью  u вернем действие.

 Теперь сохраним с помощью :wq наш файл (рис. [-@fig:006]).

![Отмена действия](image/6.png){#fig:006}
