...
Сперва посмотрим полный путь для нашего каталога (рис. [-@fig:001]).

![Определение пути до каталога](image/1.png){#fig:001}
...
Напишем ls и добавим ключ -a, таким образом -  выведем и дополнительные файлы (рис. [-@fig:003]).

![ls с ключом -a](image/3.png){#fig:003}

Теперь выведем файлы с полной информацией с помощью ключа ls  -l (рис. [-@fig:004]).
...
Создадим каталог newdir. Внутри него еще один каталог, подкаталог -  и удалим все (рис. [-@fig:009]).
...
Посмотрим все существующие ключи для cd. О (рис. [-@fig:012]).

![Ключи для cd](image/12.png){#fig:012}

Посмотрим ключи для mkdir. Основные - m (Поставить права доступа), p (Создать родительские каталоги) (рис. [-@fig:013]).
...
Выведем историю команд  с помощью history (рис. [-@fig:017]).

![Просмотр истории](image/18.png){#fig:017}




# Выводы
В ходе выполнения работы мы получили навыки работы с базовыми командами терминала
